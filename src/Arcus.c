...
int _is_performax_device(struct libusb_device_descriptor *descriptor) {
    return _is_performax_device_by_vendor_product(descriptor->idVendor,
...
int _get_libusb_device_offset_from_arcos_offset(
    libusb_device **list, ssize_t list_count, struct libusb_device_descriptor *descriptor,
...
AR_BOOL fnPerformaxComGetProductString(AR_DWORD dwNumDevice, AR_VOID *lpDeviceString,
                                       AR_DWORD dwOptions) {
    ssize_t device_count, i;
    libusb_device **list;
    struct libusb_device_descriptor descriptor;
    ...
    i = _get_libusb_device_offset_from_arcos_offset(list, device_count, &descriptor,
                                                    dwNumDevice);
    if (i < 0) {
        result =
            AR_FALSE; // invlaid dwNumDevice id; we should have found the offset above...
    } else {
...
AR_BOOL fnPerformaxComOpen(AR_DWORD dwDeviceNum, AR_HANDLE *device_handle) {
    ...
    _send_urb_control(*device_handle,
                      0x02); // Should document this better; it's some open command
    return result;
}
...
AR_BOOL fnPerformaxComClose(AR_HANDLE device_handle) {
    ...
    _send_urb_control(device_handle,
                      0x04); // Should document this better; it's some close command
...
AR_BOOL fnPerformaxComSendRecv(AR_HANDLE device_handle, AR_VOID *wBuffer,
                               AR_DWORD dwNumBytesToWrite, AR_DWORD dwNumBytesToRead,
                               AR_VOID *rBuffer) {
    if (!InitializePerformaxLibrary()) {
        return AR_FALSE;
    }
    ...
    result = libusb_bulk_transfer(device_handle, 0x82, buffer, sizeof(buffer),
                                  &transferred, libusb_ReadTimeout);
    // if the above fails, it's probably ok.  We probably don't care.

    result = libusb_bulk_transfer(device_handle, 0x02, wBuffer, dwNumBytesToWrite,
                                  &transferred, libusb_WriteTimeout);

#ifdef DEBUGARCUS
    printf("sent: %d (%d) result %d '%s'\n", transferred, (int)dwNumBytesToWrite, result,
           (char *)wBuffer);
#endif // DEBUGARCUS
    ...
    result = libusb_bulk_transfer(device_handle, 0x82, rBuffer, dwNumBytesToRead,
                                  &transferred, libusb_ReadTimeout);

#ifdef DEBUGARCUS
    printf("received: %d (%d) result %d - %s\n", transferred, (int)dwNumBytesToRead,
           result, (char *)rBuffer);
#endif // DEBUGARCUS
    if (0 != result) {
        return AR_FALSE;
...
AR_BOOL fnPerformaxComFlush(AR_HANDLE device_handle) {
    ...
    if (_send_urb_control(device_handle, 0x01) ==
        0) { // Should document this better; it's some flush command
        return AR_TRUE;
    }
    return AR_FALSE;
}

// the following does _not_ need to be called before using the other functions. It is safe
// to ignore its existence

// Since the original interface contained the above line, all functions call the
// initialization function below.  Since we have no context being passed inthe fnPerformax
// functions, we don't get the opportunity anywhere to properly close the libusb library
// with libusb_exit...
